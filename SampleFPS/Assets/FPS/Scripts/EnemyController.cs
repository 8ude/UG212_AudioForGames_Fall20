...
public class EnemyController : MonoBehaviour
{
    ...
    void OnDamaged(float damage, GameObject damageSource)
    {
        ...
        if (damageSource && damageSource.GetComponent<PlayerCharacterController>())
        {
            ...
            //fmod todo - enemy damaged sound
            //possible todo - change damage sound based on remaining health

            //use this for parameter:
            //float healthPercentage = m_Health.getRatio();

            // play the damage tick sound
            //if (damageTick && !m_WasDamagedThisFrame)
            //AudioUtility.CreateSFX(damageTick, transform.position, AudioUtility.AudioGroups.DamageTick, 0f);
            FMODUnity.RuntimeManager.PlayOneShotAttached(enemyDamagedEventRef, "EnemyHealth", m_Health.getRatio(), gameObject);
