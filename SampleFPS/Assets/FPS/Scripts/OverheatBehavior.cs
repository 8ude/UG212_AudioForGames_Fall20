...
public class OverheatBehavior : MonoBehaviour
{
    ...
    //new fmod
    [FMODUnity.EventRef] public string reloadingFMODEvent;
    ...
    FMODUnity.StudioEventEmitter reloadingEmitter;
    ...
    void Awake()
    {
        ...
        reloadingEmitter = GetComponent<FMODUnity.StudioEventEmitter>();
    }
    ...
    void Update()
    {
        ...
        // cooling sound
        if (reloadingFMODEvent != null)
        {
            if (!reloadingEmitter.IsPlaying()
                && currentAmmoRatio != 1
                && m_Weapon.isWeaponActive
                && m_Weapon.isCooling)
            {
                reloadingEmitter.Play();
            }
            else if (reloadingEmitter.IsPlaying()
                && (currentAmmoRatio == 1 || !m_Weapon.isWeaponActive || !m_Weapon.isCooling))
            {
                reloadingEmitter.Stop();
                return;
            }

            reloadingEmitter.SetParameter("CurrentAmmo", currentAmmoRatio);
        }
